...
const initModule = ({
    ...
    const genResxDistBody = (name, langData, defaultLangData) => {
        ...
        const keysToReplace = Object.keys(langData).filter(k => langData[k] === null);
        if (keysToReplace.length) {
            const dataToReplace = keysToReplace.reduce((acc, k) => {
                acc[k] = defaultLangData[k];
                return acc;
            }, {});
            distData = {
                ...distData,
        ...
        const strings = genResxStrs(distData);
        return [
            autoGenStr,
            NEW_LINE,
            genNamespaceAssign(),
    ...
    const processJsonToJs = (chunkName, body, lang) => {
            ...
            .then(srcDirData => {
                if (srcDirData.length) {
                    const genDistOps = srcDirData.map(srcFileName => {
                        ...
                        const defLangSrcPath = getDefSrcFilePath(chunkName);
                        const srcFilePath = `${srcFolder}${srcFileName}`;
                        let defaultLangData;
                        return readFileAsync(defLangSrcPath, { encoding: 'utf8' })
                            .then(defLangSrcData => {
                                defaultLangData = JSON.parse(defLangSrcData);
                            })
                            .then(() => readFileAsync(srcFilePath, { encoding: 'utf8' }))
                            .then(srcLangFileData => {
                                const langJSData = JSON.parse(srcLangFileData);
                                const resxBody = genResxDistBody(chunkName, langJSData, defaultLangData);
                                return processJsonToJs(chunkName, resxBody, lang);
                            })
                            .catch(logErr);
                    });
                    ...
                    return Promise.all(genDistOps)
                        .then(() => {
                            ...
                            return Promise.all(genTypesOps)
                                .then(() => {
                                    if (interactive) {
